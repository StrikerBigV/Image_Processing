...
int main()
{
	mat_m=220;
	mat_n=209;
	Mat imgG=Mat(mat_n,mat_m,CV_8U,Scalar::all(0));
	int cnt=0;
	for(int i=5;i<215;i++){
		for(int j=5;j<175;j++)
			imgG.at<uchar>(Point(i,j))=255;
		cnt++;
		if(cnt==7){
			i+=17;
			cnt=0;
	...
	Mat newImg=Mat(mat_n,mat_m,CV_8U,Scalar::all(0));
	for(int i=0;i<mat_m;i++)
		for(int j=0;j<mat_n;j++)
			newImg.at<uchar>(Point(i,j))=imgG.at<uchar>(Point(i,j));
	cin>>tem_m;
	cin>>tem_n;
	getchar();
